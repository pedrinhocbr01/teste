...
export class ContasService {
  ...
  async atualizar(contaId: number, dto: AtualizarContaDto) {
    const conta = await this.contaAberta(contaId);
  ...
  async removerAlocacao(contaId: number, contaItemId: number) {
    await this.contaAberta(contaId);
  ...
  async cancelarPagamento(pagamentoId: number, dto: CancelarPagamentoDto) {
    ...
    const obs = [pg.observacao, dto.motivo ? `ESTORNO: ${dto.motivo}` : 'ESTORNO']
      .filter(Boolean)
      .join(' | ');
    await this.db.execute(
